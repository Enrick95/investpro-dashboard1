...
function safeTags(tags: any, maxTags = 3) {
  ...
  return tags.slice(0, maxTags).map((t) => safeText(t, 16));
}
 ...
 */
export async function syncMt5HistoryToTrades(params: HistoryParams): Promise<number> {
  const r = await fetch("/api/mt5/history", {
    method: "POST",
  ...
  if (!r.ok || !j?.ok) throw new Error(j?.error || "history error");

  const deals: any[] = Array.isArray(j.deals) ? j.deals : [];

  // MT5: entry often 0=IN, 1=OUT (closing)
  const outDeals = deals.filter((d) => d.entry === 1);

  const trades = loadTrades();
  const existing = new Set(trades.map((t: any) => t.id));
  ...
  for (const d of outDeals) {
    ...
    trades.unshift({
      id,
      date: ymdFromUnix(Number(d.time) || 0),
      symbol: safeText(d.symbol, 24),
      market: "forex",
      side: "BUY",
      pnl: Number(pnl.toFixed(2)),
      result: resultFromPnl(pnl),
      setup: "mt5",
      tags: safeTags(["mt5", "auto"]),
      discipline: 10,
      note: safeText(`MT5 sync • pos=${d.position_id} • order=${d.order}`, 80),
    });
  ...
  if (me) {
    ...
    if (updated && (updated as any).showOnLeaderboard) {
      upsertLeaderboardUser(updated as any);
    }
  }
