...
function getLocalPlan(): PlanId {
  try {
    return (
      (localStorage.getItem("ip_plan") as PlanId) ||
      "free"
    );
  } catch {
    return "free";
...
export default function MonAbonnementPage() {
  ...
  async function openBillingPortal() {
    pushNotif({
      ...
      title: "Bientôt",
      message:
        "Billing portal pas encore branché (Stripe).",
    });
  }
        ...
        <CardBody className="space-y-4">
          ...
          <CardSubCard>
            <div className="text-sm text-[color:var(--text)]">
              • Gestion complète (Stripe) à
              brancher : renouvellement,
              factures, annulation, etc.
            </div>

            <div className="text-xs text-muted mt-2">
              Prochaine étape : connecter la
              page Abonnements à Stripe
              Checkout + Portal.
            </div>
          </CardSubCard>
